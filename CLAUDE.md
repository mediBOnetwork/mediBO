...
## 15. TOKEN DISCIPLINE (permanent — CHANGE #198)

Measured before this rule existed: 17 of 92 queue rows were auto-spawned debug
twins burning 25% of ALL tokens, 91 of 92 rows routed Opus, and nothing ever ran
below high effort. Five levers fixed that. Every knob lives in
`dev_runner_config.worker_pool` — change it with `pool_set()`, never a deploy.

1. **Read `REPO_MAP.md` FIRST — never scan the repo.** It is a one-line-per-file
   index (path → purpose → exports → RPCs it calls) of every screen, service,
   migration, edge function, script and protected test, regenerated by
   `deploy.sh` on every deploy via `scripts/gen_repo_map.sh`, so it is never
   stale. Find the ≤5 files your command touches and open ONLY those. A
   `grep -r` / `rg` across all of `lib/` was costing 200–400k input tokens on
   jobs that changed one widget. A repo-wide scan is allowed ONLY when a map
   lookup genuinely fails — and then say so in the build log so the map is fixed.

2. **A debug pass is EVIDENCE, not habit.** `_dev_auto_debug_trg` creates a
   "Debug pass — verify & fix #N" twin only when `qa_status='failed'`, a linked
   `dev_journey_runs` row came back red, or Om explicitly asked
   (`dev_cmd_request_debug`). QA passed = done, `debug_status='not_needed'`, no
   twin, no second bill. Never restore blanket twinning.

3. **Routing is three lanes.** `_route_detect` returns
   `(route, area, size_class, reason)`: **haiku** for <400-char single-file jobs,
   **sonnet** under 2000 chars with no schema/migration/payment/auth marker,
   **opus** only for genuinely multi-system work. Keep `opus_markers` NARROW — it
   once held 'rpc'/'engine'/'pipeline'/'end-to-end'/'realtime', which matched 91
   of 92 specs and vetoed the cheap lanes on everything. Every row stores
   `route_reason`, rendered verbatim on the command detail screen so a misroute
   is visible instead of buried in a log.

4. **Standard effort is the default.** `_dev_effort_for()` returns `standard`;
   the backend escalates to `high` itself on a retry, on `size_class='xlarge'`,
   or when the spec literally says `effort: high`. Never silently work harder
   than the row asked for — that setting is what Om is paying.

5. **Batch the small stuff.** `devcmd.sh claim_batch <agent> [routes] [area]
   [max]` claims up to `worker_pool.batch_max` SMALL commands from the SAME area
   — one worker boot, one repo read, one deploy-lane pass. Finish every row with
   its own `complete`/`fail`: a failing sub-item is failed alone and never sinks
   its siblings.
