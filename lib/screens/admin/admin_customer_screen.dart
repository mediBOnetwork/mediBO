...
class _AdminCustomerScreenState extends State<AdminCustomerScreen> {
  ...
  Widget _buildDesktopCustRow(_CustRow row) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      // CHANGE #462 — per-order payment panel (shared with the customer's
      // Payment tab — same widget, same RPC, never disagrees)
      if (row.orderId != null && _payOpen[row.orderId] == true)
        Padding(
          padding: const EdgeInsets.fromLTRB(20, 12, 20, 16),
          child: PaymentPanel(
            key: ValueKey(row.orderId),
            orderId: row.orderId!,
    ...
    return Container(
      ...
      child: GestureDetector(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          // CHANGE #462 — per-order payment panel (mobile) — shared with the
          // customer's Payment tab
          if (row.orderId != null && _payOpen[row.orderId] == true)
            GestureDetector(
              behavior: HitTestBehavior.opaque,
              onTap: () {},
