...
class _LoginPanelContentState extends State<_LoginPanelContent> {
  ...
  // ── Magic link ────────────────────────────────────────────────────────────
  final _mlEmailCtrl = TextEditingController();
  ...
  Future<void> _sendMagicLink() async {
    final email = _mlEmailCtrl.text.trim();
    if (email.isEmpty) {
  ...
  Future<void> _adminSignIn() async {
    ...
    try {
      ...
      // panel closes via _authSub signedIn listener; _AppRoot routes to AdminShell
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // ── Magic link ──────────────────────────────────────────
                if (_mlSent)
                  _LoginPanelMagicSentCard(
                    email: _mlEmailCtrl.text.trim(),
                    onResend: () => setState(() { _mlSent = false; _mlError = null; }),
                  )
                else ...[
                  TextField(
                    controller: _mlEmailCtrl,
                ...
                // ── Admin dev login ─────────────────────────────────────
                const SizedBox(height: 28),
                _LoginPanelAdminCard(
                  expanded: _showAdminLogin,
                  onToggle: () => setState(() { _showAdminLogin = !_showAdminLogin; _adminError = null; }),
...
class _LoginPanelMagicSentCard extends StatelessWidget {
  final String email;
  final VoidCallback onResend;
  const _LoginPanelMagicSentCard({required this.email, required this.onResend});
...
class _LoginPanelAdminCard extends StatelessWidget {
  final bool expanded;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        InkWell(
          ...
          child: Padding(
            ...
            child: Row(children: [
              const Icon(Icons.admin_panel_settings_outlined, size: 16, color: Color(0xFF6B7280)),
              const SizedBox(width: 8),
