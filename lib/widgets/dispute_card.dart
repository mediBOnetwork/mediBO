...
class DisputeCard extends StatelessWidget {
  ...
  Widget _returnNoteChip() {
    final isOpen = item.returnNoteStatus == 'open';
    RenderLog.write('c350_return_chip', 'state=${item.returnNoteStatus}');
    return Align(
      alignment: Alignment.centerLeft,
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
        decoration: BoxDecoration(
          color: isOpen ? _kAmberBg : _kNeutral,
          borderRadius: BorderRadius.circular(20),
        ),
        child: Text(
          isOpen ? 'Stock with buyer — pick up on next delivery' : 'Return collected ✓',
          style: TextStyle(
