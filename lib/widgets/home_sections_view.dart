      ...
      child: Column(
        ...
        children: [
          ...
          switch (section.layout) {
            ...
            // CHANGE #177 — near-expiry supplier deals with disclosure gate.
            HomeSectionLayout.shortDated => _ShortDatedRail(section: section),
...
/// CHANGE #177 — horizontal rail of near-expiry supplier deals.
///
/// The backend controls whether this section appears (active flag + has_offers),
/// the discount percentage, expiry label, and the disclosure copy. The rail
/// only renders; no business logic lives here.
///
/// Tapping a card shows [_DisclosureSheet] before any cart action — the RPC
/// itself enforces `p_disclosure_seen=true` as an extra guard.
class _ShortDatedRail extends StatelessWidget {
  final HomeSection section;
  const _ShortDatedRail({required this.section});

  static const double _cardW = 172;
  static const double _cardH = 188;
  static const double _gap = 12;

  @override
  Widget build(BuildContext context) {
    final offers = section.shortDatedOffers;
    return SizedBox(
      height: _cardH,
      child: ListView.builder(
        scrollDirection: Axis.horizontal,
        ...
        itemBuilder: (context, i) {
          ...
          return Padding(
            ...
            child: _ShortDatedCard(
              offer: offer,
              disclosureNote: section.disclosureNote,
            ),
          );
        },
      ),
...
class _ShortDatedCard extends StatelessWidget {
  final ShortDatedOffer offer;
  final String disclosureNote;
  const _ShortDatedCard({required this.offer, required this.disclosureNote});

  @override
  Widget build(BuildContext context) {
    final warn = offer.expiryWarning;
    final warnBg = Brand.hex(warn['bg']?.toString() ?? '', Brand.accentSoft);
    final warnFg = Brand.hex(warn['fg']?.toString() ?? '', Brand.green);
    final warnLabel = warn['label']?.toString() ?? offer.monthsLabel;
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            // discount badge
            Container(
              padding: EdgeInsets.symmetric(horizontal: Ds.space.x8, vertical: 3),
              decoration: BoxDecoration(
                color: Brand.accentSoft,
              ...
              child: Text(
                offer.discountLabel,
                style: AppType.l5.copyWith(color: Brand.green),
              ),
            ),
            const SizedBox(height: 8),
            // product name
            ...
            // expiry chip
            Container(
              padding: EdgeInsets.symmetric(horizontal: Ds.space.x4 + 2, vertical: 2),
              decoration: BoxDecoration(
                color: warnBg,
              ...
              child: Text(
                warnLabel,
                style: AppType.t2.copyWith(color: warnFg),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
              ),
            ),
            ...
            // remaining qty
            Text(
              'Qty: ${offer.remainingQty.toStringAsFixed(0)}',
              style: AppType.t1,
              maxLines: 1,
            ),
          ],
...
/// Bottom sheet shown before a short-dated add-to-cart. Prints the backend's
/// disclosure_title and disclosure_body verbatim; the RPC enforces the same
/// gate server-side.
class _DisclosureSheet extends StatefulWidget {
  final ShortDatedOffer offer;
  final String disclosureNote;
  const _DisclosureSheet({required this.offer, required this.disclosureNote});

  @override
  State<_DisclosureSheet> createState() => _DisclosureSheetState();
}

class _DisclosureSheetState extends State<_DisclosureSheet> {
  int _qty = 1;
  bool _loading = false;
  String? _error;
  ...
  Future<void> _addToCart() async {
    ...
    try {
      await Supabase.instance.client.rpc('short_dated_add_to_cart', params: {
        'p_offer_id': widget.offer.offerId,
        'p_qty': _qty,
        'p_disclosure_seen': true,
      });
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    final maxQty = offer.remainingQty.toInt().clamp(1, 999);

    return SafeArea(
      child: Container(
        margin: EdgeInsets.fromLTRB(Ds.space.x12, 0, Ds.space.x12, Ds.space.x12),
        decoration: BoxDecoration(
          color: Ds.c.surface,
          borderRadius: BorderRadius.circular(Ds.r.card),
        ),
        padding: EdgeInsets.fromLTRB(Ds.space.x16, Ds.space.x16, Ds.space.x16, Ds.space.x24),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            Center(
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: Brand.border,
                  borderRadius: BorderRadius.circular(Ds.space.x4 / 2),
                ),
              ),
            ),
            const SizedBox(height: 16),
            Text(offer.disclosureTitle, style: AppType.l2),
            const SizedBox(height: 8),
            Text(offer.disclosureBody, style: AppType.b2),
            if (widget.disclosureNote.isNotEmpty) ...[
              const SizedBox(height: 8),
              Text(widget.disclosureNote,
                  style: AppType.t1.copyWith(color: Brand.amber)),
            ],
            const SizedBox(height: 20),
            // product name + batch
            Text(offer.productName, style: AppType.l3),
            const SizedBox(height: 2),
            Text('Batch: ${offer.batchNo}  ·  Expires: ${offer.batchExpiry}',
                style: AppType.t1),
            const SizedBox(height: 16),
            // qty row
            Row(
              children: [
                Text('Quantity', style: AppType.l4),
                const Spacer(),
                IconButton(
                  onPressed: _qty > 1
                      ? () => setState(() => _qty--)
            ...
            if (_error != null) ...[
              const SizedBox(height: 8),
              Text(_error!, style: AppType.b3.copyWith(color: Brand.danger)),
            ],
            const SizedBox(height: 16),
            SizedBox(
              width: double.infinity,
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(Ds.r.button - 2)),
                ),
                child: _loading
                    ? const SizedBox(
                        width: 20,
                    ...
                    : Text('I understand — Add to cart',
                        style: Ds.t.body.copyWith(fontWeight: FontWeight.w600, color: Colors.white)),
              ),
            ),
          ],
        ),
