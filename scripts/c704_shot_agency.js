#!/usr/bin/env node
/**
 * c704_shot_agency.js - pixel proof for the AGENCY DISPATCH board (CHANGE #704).
 *
 * render_verify.js can drive the logged-in ADMIN to a route (--admin-path) and
 * shot.sh can capture a public URL. Neither can photograph a screen only an
 * AGENCY OWNER can reach, and the #704 dispatch board is exactly that. This
 * signs in as the documented agency test credential
 * (test.agency1@medibo.in / TestAgency1#26 - a synthetic agency living in the
 * synthetic zone 99, so it can never be handed a real customer's order), opens
 * /agency/dispatch, waits for Flutter to paint and saves the pixels.
 *
 *   node scripts/c704_shot_agency.js --out proof.png [--path /agency/dispatch]
 *       [--wheel 600] [--width 430] [--height 1200]
 *       [--wait-key c704_agency_dispatch]
 *
...
const EMAIL = process.env.MEDIBO_AGENCY_EMAIL || 'test.agency1@medibo.in';
const PASS  = process.env.MEDIBO_AGENCY_PASS  || 'TestAgency1#26';
...
(async () => {
  ...
  const path   = val('--path', '/agency/dispatch');
  if (!out) { console.error('c704_shot_agency: --out <file.png> is required'); process.exit(1); }
  ...
  try {
    ...
    console.error(`c704_shot_agency: ${e.message}`);
