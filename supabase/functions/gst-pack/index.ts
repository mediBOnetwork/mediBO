// gst-pack — CMD #416: the month's GST paperwork as one PDF a CA can work from.
//
// A DUMB PRINTER. Every heading, every rupee string, every table row and the
// disclaimer at the bottom arrive finished from pharmacy_gst_pack_input(); this
// file decides nothing about tax, wording or layout beyond where the ink goes.
// The blocks are the SAME blocks the screen renders, so the PDF and the screen
// can never disagree about what was filed.
//
// It never claims a return was filed. The disclaimer it prints is the backend's
// `pgst.filing_note`, which says plainly that filing happens on the portal.
//
// Same request → render → report shape as pos-invoice (#411): the screen polls
// pharmacy_gst_pack_status() and this reports the result back onto the run row.
...
async function renderPack(input: any): Promise<Uint8Array> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: input, error: inErr } =
      await supabase.rpc('pharmacy_gst_pack_input', { p_run_id: runId })
    if (inErr) throw new Error('pharmacy_gst_pack_input: ' + inErr.message)
    if (!input?.ok) {
      await supabase.rpc('pharmacy_gst_pack_report', {
        p_run_id: runId, p_ok: false,
        p_error: 'pack_input: ' + (input?.error ?? 'unknown'),
      }).catch(() => {})
      return json({ ok: false, reason: input?.error ?? 'no_input' })
    }

    const bytes = await renderPack(input)
    const up = await supabase.storage.from(String(input.bucket))
      .upload(String(input.path), bytes,
              { contentType: 'application/pdf', upsert: true })
    if (up.error) throw new Error('upload: ' + up.error.message)

    const { data: rep } = await supabase.rpc('pharmacy_gst_pack_report', {
      p_run_id: runId, p_ok: true, p_bucket: input.bucket,
      p_path: input.path, p_name: input.name, p_bytes: bytes.length,
    })
    return json({ ok: true, run_id: runId, path: input.path,
                  name: input.name, bytes: bytes.length, report: rep })
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    if (runId) {
      await supabase.rpc('pharmacy_gst_pack_report',
        { p_run_id: runId, p_ok: false, p_error: msg }).catch(() => {})
    }
