-- CHANGE #294 (part E3) — where each media header actually comes from, plus the
-- bill send moved onto the same window gate as everything else.

create or replace function public.wa_event_header_media(p_event_key text, p_order_id uuid)
returns jsonb
language plpgsql stable security definer set search_path to 'public'
as $$
declare v_url text; v_bucket text; v_path text; v_name text;
begin
  if p_order_id is null then return null; end if;

  if p_event_key = 'payment_qr' then
    -- Razorpay hosts the QR itself, so Meta can fetch it straight from the link.
  ...
  if p_event_key = 'bill_to_customer' then
    -- The bill lives in a PRIVATE bucket: hand the sender the object, not a URL,
    -- and let it sign a short-lived link at send time.
    select coalesce(o.cust_bill_bucket,'customer-bills'), o.cust_bill_path,
           coalesce(o.cust_bill_name,'Bill.pdf')
      into v_bucket, v_path, v_name
      from orders o where o.id = p_order_id;
    if coalesce(v_path,'') <> '' then
      return jsonb_build_object('type','document','bucket',v_bucket,'path',v_path,'filename',v_name);
    end if;
    return null;
  end if;
...
-- A storage-backed header is just as valid as a link one.
...
declare r record; v_lang text; v_tid uuid; v_ph text; v_cid uuid; v_rid uuid;
        ...
        v_varmap jsonb; v_tok jsonb; v_key text; v_val text; v_missing text[] := '{}';
        v_cust uuid := p_customer_id; v_hdr_fmt text; v_hdr jsonb;
begin
  ...
  select coalesce(
           (select jsonb_agg('{{'||k||'}}') from jsonb_array_elements_text(tt.token_map) k),
           r.variable_map, '[]'::jsonb)
    into v_varmap from wa_templates tt where tt.id = v_tid;
  ...
  loop
    if v_key is not null and not (v_tok ? v_key) then
      ...
      if v_val is null then v_missing := v_missing || v_key;
      else v_tok := v_tok || jsonb_build_object(v_key, v_val); end if;
    end if;
  end loop;
...
-- The bill send now obeys the same window rule as every other customer message.
...
begin
  ...
  return public.wa_notify_customer_event(
    'bill_to_customer', p_order_id, v_ph10,
