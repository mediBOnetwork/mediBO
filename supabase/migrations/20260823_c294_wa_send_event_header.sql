-- CHANGE #294 (part E2) — wa_send_event resolves and stores the per-message media
-- header, and refuses (so the fallback route can take over) when a media-header
-- template has no media to put in it. Body is otherwise byte-for-byte the
-- pre-#294 function.
...
begin
  ...
  -- CHANGE #294 — a media HEADER must be filled per message. No media, no send:
  -- returning here lets wa_send_event_or_fallback reach for the fallback route
  -- instead of Meta rejecting a header-less template call.
  ...
  if v_hdr_fmt is not null then
    ...
    if v_hdr is null or coalesce(v_hdr->>'link','') = '' then
    ...
    v_hdr := jsonb_build_object('type', v_hdr_fmt, 'link', v_hdr->>'link');
  end if;
