-- CHANGE #300 — what the live signed-delivery test found.
--
-- All 53 ticked events were replayed against the real endpoint with genuine
-- HMAC-SHA256 signatures. Two things were wrong, both in the qr_code.closed
-- branch, and one in the duplicate path:
--
-- 1. A late qr_code.closed on an ALREADY-PAID QR kept status='paid' but
--    overwrote closed_reason with the close event's reason ('on_demand').
--    closed_reason is exported by rzp_qr_view() and reaches the app, so a paid
--    QR could end up labelled as if it had been closed unpaid. Razorpay DOES
--    send qr_code.closed after a single-use QR is credited, so this is the
--    normal sequence, not a corner case. A paid QR now keeps its own reason.
--
-- 2. qr_code.closed for a QR we have never seen logged handled=true even though
--    it matched zero rows — the operator log claimed a delivery was acted on
--    when nothing happened. handled now reflects whether a row was actually
--    touched, and the reply carries the matched count.
--
-- 3. The duplicate path's exception handler sat on the function's OUTER block,
--    so a unique_violation rolled back the razorpay_webhook_log insert with it
--    and the delivery vanished from the log. The claim insert now has its own
--    sub-block, and the outer handler re-records the delivery.
