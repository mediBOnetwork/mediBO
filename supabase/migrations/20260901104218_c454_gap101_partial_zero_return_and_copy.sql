-- CMD #454 — gap #101 follow-through: the copy key the message reads, and the
-- zero-return case (a "partial" that returns nothing is just a completion and
-- must not be refused for naming no lines).

insert into public.ui_copy(key, value) values
  ('delivery.partial_recorded', to_jsonb('Partial delivery recorded — {qty} item(s) coming back'::text))
...
begin
  ...
  -- nothing coming back → an ordinary completion, unchanged behaviour
