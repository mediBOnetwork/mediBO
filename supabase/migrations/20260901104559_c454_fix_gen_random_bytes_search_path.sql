-- CMD #454 — issue auto-solved while proving gap #100.
-- _delivery_assign_core is SET search_path TO 'public' but called BARE
-- gen_random_bytes(), which lives in the `extensions` schema on this project.
-- Every assignment therefore raised 42883 on the qr_token expression — which is
-- why `deliveries` held zero rows. Schema-qualified.
