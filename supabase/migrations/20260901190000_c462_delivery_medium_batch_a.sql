...
-- ─────────────────────────────────────────────────────────────────────────────
-- 11. GAP 106, the other half — a flag nobody can see is not a fix.
--     The completions that landed outside the geofence become a section on the
--     admin delivery ops screen, fully worded by the backend, with an
--     acknowledge action. The screen renders sections in payload order and
--     skips a key it does not know, so this is additive by construction.
-- ─────────────────────────────────────────────────────────────────────────────
insert into public.ui_copy(key, value) values
  ('admin.delivery.geofence_title',     '"Delivered away from the address"'::jsonb),
  ('admin.delivery.geofence_empty',     '"Every delivery today was confirmed at the address."'::jsonb),
  ('admin.delivery.geofence_clear_btn', '"Reviewed"'::jsonb),
  ('admin.delivery.geofence_chip',      '"Off-site"'::jsonb)
on conflict (key) do nothing;

create or replace function public._c462_geofence_section()
returns jsonb language sql stable security definer set search_path to 'public' as $function$
  select jsonb_build_object(
    'key','geofence',
    'title', public._c('admin.delivery.geofence_title'),
    'empty', public._c('admin.delivery.geofence_empty'),
    'rows', coalesce((
      select jsonb_agg(jsonb_build_object(
               'delivery_id',    d.id,
               'partner_name',   coalesce(p.full_name,''),
               'order_code',     coalesce(o.order_code,''),
               'distance_label', d.delivered_distance_m::text || ' m from the address'
                                 || case when cfg.radius is null then ''
                                         else ' · limit ' || cfg.radius::text || ' m' end,
               'when_label',     to_char(d.delivered_at at time zone 'Asia/Kolkata','DD Mon, hh12:mi am'),
               'status_chip',    public._c('admin.delivery.geofence_chip'),
               'status_colors',  jsonb_build_object('bg','#FEF3C7','fg','#92400E'),
               'can_clear',      true,
               'clear_label',    public._c('admin.delivery.geofence_clear_btn'))
             order by d.delivered_at desc)
        from public.deliveries d
        left join public.delivery_partner_registrations p on p.id = d.partner_id
        left join public.orders o on o.id = d.order_id
        cross join lateral (select (public._dcfg(d.zone_id)->>'geofence_radius_m')::int radius) cfg
       where d.completion_flagged
         and d.delivered_at >= now() - interval '14 days'), '[]'::jsonb));
$function$;

create or replace function public.admin_delivery_geofence_clear(p_delivery_id uuid)
returns jsonb language plpgsql security definer set search_path to 'public' as $function$
begin
  if not public._is_admin() then
    return jsonb_build_object('ok',false,'error','not_authorized');
  end if;
  update public.deliveries set completion_flagged = false where id = p_delivery_id;
  if not found then return jsonb_build_object('ok',false,'error','not_found'); end if;
  insert into public.delivery_events(delivery_id, order_id, partner_id, event, note, actor)
  select p_delivery_id, d.order_id, d.partner_id, 'geofence_reviewed',
         d.delivered_distance_m::text || 'm', coalesce(auth.jwt()->>'email','admin')
    from public.deliveries d where d.id = p_delivery_id;
  return jsonb_build_object('ok',true,'message', public._c('admin.delivery.geofence_clear_btn'));
end $function$;

grant execute on function public.admin_delivery_geofence_clear(uuid) to authenticated;

-- admin_delivery_ops() gains the section. Everything else is byte-for-byte the
-- definition that was live before CHANGE #462.
CREATE OR REPLACE FUNCTION public.admin_delivery_ops(p_zone smallint DEFAULT NULL::smallint)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  return jsonb_build_object(
    ...
    'sections', jsonb_build_array(
      ...
      -- CHANGE #462 (gap 106): the completions that landed outside the geofence.
      public._c462_geofence_section()),
