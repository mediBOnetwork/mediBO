-- CHANGE #470 — exceptions_queue v2: every row names the STAGE it is stuck at
-- and carries a one-tap link to the screen that fixes it. The route is resolved
-- ONCE (action_route, else exception_route_map, else the ops-board class) so the
-- button and the link can never point at different screens.

CREATE OR REPLACE FUNCTION public.exceptions_queue(p_zone smallint DEFAULT NULL::smallint, p_reason text DEFAULT NULL::text, p_status text DEFAULT 'open'::text, p_limit integer DEFAULT 200)
 RETURNS jsonb
 LANGUAGE plpgsql
