-- CHANGE #1674, QA round — the batch-window sentence could never reach the card.
--
-- deploy_lane_status() built window_label as a TOP-LEVEL key, while
-- deploy_lane_section.dart reads queue['window_label'] and
-- build_speed_test.dart's fixture nests it under queue. The screen and its
-- protected test agreed with each other and both disagreed with the server, so
-- the test stayed green on its own fixture while the live lane card silently
-- dropped the line — the backend had been writing "Batch window 90s · nothing
-- waiting." since the change shipped and no one could see it.
--
-- One key moved inside `queue`. Nothing else in the payload changes.
