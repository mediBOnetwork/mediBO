...
--
-- The whole file is fenced behind the app-schema sentinel for the same reason
-- as its sibling: CHANGE #1802 replays every migration on the CONTROL PLANE
-- too, and medibo-dev carries cron_task without the storefront schema, so an
-- unguarded run would register a task there pointing at a function that
-- database does not have — and fail the batch on the way.

do $c668fix$
declare v jsonb; v_int int; v_pin time;
begin
  if to_regprocedure('public.test_customer_shop_ensure()') is null
     or to_regprocedure('public.identity_norm(text)') is null
    ...
    raise notice 'c668: app schema absent here (control plane) — no shelf to repair';
    return;
  end if;
  ...
  -- The row is created by 20260905170500 / 20260905183000; if neither has been
  -- replayed on this database yet, create it here so the orders of replay agree.
