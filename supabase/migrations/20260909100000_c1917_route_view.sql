-- CMD #1917 — ONE route payload for BOTH screens.
--
-- Today (routes_today) and All plans (route_plan_get) had grown two separate
-- pictures of the same route: All plans had the map, the "Stops 1-10" jump
-- chips, the photo/score stop card, the five actions, the route cost and the
-- closed-at-arrival warning; Today had a progress line, a Navigate button and
-- a bare text stop row. That drift is the bug — two implementations of the
-- same thing always diverge.
--
-- route_view(route_id) is now the single answer. It carries the header (title,
-- worker, cost chips, warnings, Assign / Message stops / Navigate), the map
-- payload, the jump windows and the stop rows — every stop with its photo,
-- score chip, status chip and the SAME five actions on both screens:
-- Call, WhatsApp, Navigate, Check in, Import customer.
--
-- Also here: route_map_load, so "the map loaded once for this route" is a fact
-- with a number on it rather than a claim.
--
-- Idempotent: CREATE TABLE IF NOT EXISTS, CREATE OR REPLACE, ON CONFLICT DO
-- NOTHING.

-- ── 1. Copy (wording is an UPDATE, never a deploy) ────────────────────────
insert into ui_copy(key, value) values
  ('route_view.stops_title',      '"Stops"'::jsonb),
  ('route_view.window',           '"Stops {from}–{to}"'::jsonb),
  ('route_view.act_call',         '"Call"'::jsonb),
  ('route_view.act_whatsapp',     '"WhatsApp"'::jsonb),
  ('route_view.act_navigate',     '"Navigate"'::jsonb),
  ('route_view.act_checkin',      '"Check in"'::jsonb),
  ('route_view.act_recheckin',    '"Change"'::jsonb),
  ('route_view.act_import',       '"Import customer"'::jsonb),
  ('route_view.act_imported',     '"Customer"'::jsonb),
  ('route_view.no_phone',         '"No phone number on this lead"'::jsonb),
  ('route_view.no_coords',        '"No coordinates on this lead"'::jsonb),
  ('route_view.already_customer', '"Already a customer"'::jsonb),
  ('route_view.assign',           '"Assign"'::jsonb),
  ('route_view.assigned',         '"Assigned"'::jsonb),
  ('route_view.map_expand',       '"Tap the map to enlarge"'::jsonb),
  ('route_view.map_collapse',     '"Tap the map to shrink"'::jsonb),
  ('route_view.map_none',         '"Nothing to map on this route yet."'::jsonb),
  ('route_view.locked',           '"This route is not in the active zone."'::jsonb),
  ('route_view.stops_empty',      '"No stops on this route yet."'::jsonb),
  ('route_view.map_loads_one',    '"Map loaded once"'::jsonb),
  ('route_view.map_loads_many',   '"Map loaded {n} times"'::jsonb),
  ('route_view.day_warning',      '"Longer than an 8-hour day"'::jsonb),
  ('route_view.closed_count',     '"{n} shut on arrival"'::jsonb),
  ('route_view.map_mini_h',       '180'::jsonb),
  ('route_view.map_large_vh',     '0.60'::jsonb)
on conflict (key) do nothing;

-- ── 2. Who may see a route ────────────────────────────────────────────────
-- Zone-scoped for everyone (admin_active_zone()). DATE-scoped for a worker:
-- a rep only ever opens his own route for admin_active_date(). An admin on
-- All plans is browsing plans that carry their own date, so the date fence
-- would make every past plan unopenable — the zone fence still holds.
create or replace function public._c1917_route_ok(p_route_id uuid)
...
-- ── 3. Map loads, counted ─────────────────────────────────────────────────
-- Rule 4 of the spec: "Log map loads per route so the cost is visible." One
-- row per route per day per screen; the count is what proves the map is
-- created once and only resized afterwards.
create table if not exists public.route_map_load (
  route_id   uuid        not null,
  day        date        not null default (now() at time zone 'Asia/Kolkata')::date,
  screen     text        not null default 'route_view',
  loads      integer     not null default 0,
  first_at   timestamptz not null default now(),
  last_at    timestamptz not null default now(),
  primary key (route_id, day, screen)
);

create or replace function public.route_map_loaded(p_route_id uuid, p_screen text default 'route_view')
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_loads integer; v_total integer;
begin
  if not public._c1917_route_ok(p_route_id) then
    return jsonb_build_object('ok', false);
  end if;
  insert into route_map_load(route_id, screen, loads)
  values (p_route_id, coalesce(nullif(btrim(p_screen), ''), 'route_view'), 1)
  on conflict (route_id, day, screen) do update
    set loads = route_map_load.loads + 1, last_at = now()
  returning loads into v_loads;

  select coalesce(sum(loads), 0) into v_total
    from route_map_load
   where route_id = p_route_id
     and day = (now() at time zone 'Asia/Kolkata')::date;

  return jsonb_build_object(
    'ok', true, 'route_id', p_route_id, 'loads', v_loads, 'loads_today', v_total,
    'label', case when v_total = 1
               then coalesce(nullif(public._c('route_view.map_loads_one'), ''), 'Map loaded once')
               else replace(coalesce(nullif(public._c('route_view.map_loads_many'), ''),
                                     'Map loaded {n} times'), '{n}', v_total::text) end);
end;
$$;

-- ── 4. route_view(route_id) — the one payload both screens render ─────────
...
begin
  ...
  return jsonb_build_object(
    ...
    'map',         public.route_map(p_route_id),
...
grant execute on function public._c1917_route_ok(uuid)        to authenticated;
grant execute on function public.route_view(uuid)             to authenticated;
grant execute on function public.route_map_loaded(uuid, text) to authenticated;

alter table public.route_map_load enable row level security;
do $rls$
begin
  if not exists (select 1 from pg_policies
                  where schemaname='public' and tablename='route_map_load'
                    and policyname='route_map_load_read') then
    create policy route_map_load_read on public.route_map_load
