...
Map<String, dynamic> _tab(String key, String label, String route,
        {bool visible = true, String icon = 'people'}) =>
    {
      'key': key,
      'label': label,
      'icon_key': icon,
...
Map<String, dynamic> _tile(String key, String label, String route,
        {int badge = 0, String badgeLabel = '', String desc = ''}) =>
    {
      'feature_key': key,
      'label': label,
      'icon_key': 'receipt',
...
final Map<String, dynamic> _moneyPayload = {
  ...
  'sections': [
    ...
    },
    {
      ...
      'sublabel': '',
      'items': [
        _tile('admin.bill_pipeline', 'Bills', 'bill_pipeline',
            badge: 7, badgeLabel: '7 bills to review'),
      ],
    },
  ],
  'items_count': 3,
...
final Map<String, dynamic> _morePayload = {
  ...
  'sections': [
    ...
    },
    {
      ...
      'sublabel': '',
      'items': [
        _tile('admin.audit_log', 'Audit trail', 'audit_log',
...
void main() {
  ...
  group('StaffHomeScreen — one home, rendered verbatim', () {
    testWidgets('Money: sections + tiles in payload order, stats verbatim, tap hands the map back',
        (tester) async {
      Map<String, dynamic>? opened;
      await tester.pumpWidget(_host(StaffHomeScreen(
        tabKey: 'money',
      ...
      expect(find.text('Bills, payments, settlements and the books.'),
          findsOneWidget);
      // stats verbatim
    ...
    testWidgets('More: search filters the backend list; recents and the report button only when sent',
        (tester) async {
      var report = 0;
      await tester.pumpWidget(_host(StaffHomeScreen(
        tabKey: 'more',
      ...
      await tester.enterText(find.byKey(const Key('c1016_more_search')), 'audit');
      await tester.pump();
      ...
      await tester.enterText(find.byKey(const Key('c1016_more_search')), 'zzz');
      await tester.pump();
    ...
    testWidgets('the Money home does not offer the unused report', (tester) async {
      await tester.pumpWidget(_host(StaffHomeScreen(
        tabKey: 'money',
        load: (_) async => _moneyPayload,
        onOpen: (_) {},
  ...
  group('StaffHomeStrip — the extras beside a page', () {
    testWidgets('chips in payload order, tap hands the map back', (tester) async {
      Map<String, dynamic>? opened;
      await tester.pumpWidget(_host(StaffHomeStrip(
        tabKey: 'fulfill',
        load: (_) async => {
          ..._moneyPayload,
          'tab_key': 'fulfill',
          'sections': [
            {
              ...
              'items': [
                ...
                _tile('worker.my_tasks', 'My tasks', 'my_tasks', badge: 3,
                    badgeLabel: '3 tasks'),
              ],
            },
            {
              ...
              'label': 'Delivery',
              'items': [
                _tile('admin.delivery_waves', 'Delivery waves', 'delivery_waves'),
              ],
            },
          ],
          'items_count': 3,
    ...
    testWidgets('an empty answer draws nothing at all', (tester) async {
      await tester.pumpWidget(_host(StaffHomeStrip(
        tabKey: 'customers',
        load: (_) async => {..._moneyPayload, 'sections': [], 'items_count': 0},
        onOpen: (_) {},
      )));
      await tester.pump();
  ...
  group('shell_staff_routes — the partner doors', () {
    test('the seven partner routes resolve; an unknown route resolves to null',
        () {
      ...
      expect(shellStaffRouteScreen('settlement'), isNull,
